...
describe Mumukit::Inspection do
  ...
  it { expect(Mumukit::Inspection.parse('HasUsage:=m').to_h).to eq(type: 'HasUsage', negated: true, target: 'm') }
  it { expect(Mumukit::Inspection.parse('HasUsage:~m').to_h).to eq(type: 'HasUsage', negated: true, target: 'm', like: true) }
  it { expect(Mumukit::Inspection.parse('HasUsage:*').to_h).to eq(type: 'HasUsage', negated: true) }
